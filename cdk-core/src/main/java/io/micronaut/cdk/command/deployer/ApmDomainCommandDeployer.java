...
import io.micronaut.cdk.action.spec.ApmDomainSpec;
...
import io.micronaut.cdk.component.ApmDomain;
import io.micronaut.cdk.component.Component;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link ApmDomainSpec}.
 */
public interface ApmDomainCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof ApmDomainSpec)) {
            return null;
        }

        return deployApmDomain(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployApmDomain(command);
    }
     ...
     */
    @NonNull
    ApmDomain<?> deployApmDomain(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployApmDomain(@NonNull Command command) throws CommandRunException;
}
