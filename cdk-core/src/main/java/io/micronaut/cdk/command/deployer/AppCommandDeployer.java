...
import io.micronaut.cdk.action.delete.AppDeleteSpec;
import io.micronaut.cdk.action.spec.AppSpec;
...
import io.micronaut.cdk.component.App;
import io.micronaut.cdk.component.Component;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link AppSpec}.
 */
public interface AppCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof AppSpec)) {
            return null;
        }

        return deployApp(command);
    }

    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (!command.isDelete() || !(command.getDeleteSpec() instanceof AppDeleteSpec)) {
            return false;
        }

        return undeployApp(command);
    }
     ...
     */
    @NonNull
    App<?> deployApp(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployApp(@NonNull Command command) throws CommandRunException;
}
