...
import io.micronaut.cdk.action.spec.ApplicationSpec;
...
import io.micronaut.cdk.component.Application;
import io.micronaut.cdk.component.Component;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link ApplicationSpec}.
 */
public interface ApplicationCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof ApplicationSpec)) {
            return null;
        }

        return deployApplication(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployApplication(command);
    }
     ...
     */
    @NonNull
    Application<?> deployApplication(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployApplication(@NonNull Command command) throws CommandRunException;
}
