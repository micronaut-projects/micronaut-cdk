...
import io.micronaut.cdk.action.spec.CognitoUserPoolSpec;
...
import io.micronaut.cdk.component.CognitoUserPool;
import io.micronaut.cdk.component.Component;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link CognitoUserPoolSpec}.
 */
public interface CognitoUserPoolCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof CognitoUserPoolSpec)) {
            return null;
        }

        return deployCognitoUserPool(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployCognitoUserPool(command);
    }
     ...
     */
    @NonNull
    CognitoUserPool<?> deployCognitoUserPool(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployCognitoUserPool(@NonNull Command command) throws CommandRunException;
}
