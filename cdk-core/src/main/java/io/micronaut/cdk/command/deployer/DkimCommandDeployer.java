...
import io.micronaut.cdk.action.spec.DkimSpec;
import io.micronaut.cdk.action.delete.DkimDeleteSpec;
...
import io.micronaut.cdk.component.Dkim;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link DkimSpec}.
 */
public interface DkimCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof DkimSpec)) {
            return null;
        }

        return deployDkim(command);
    }

    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (!command.isDelete() || !(command.getDeleteSpec() instanceof DkimDeleteSpec)) {
            return false;
        }

        return undeployDkim(command);
    }
     ...
     */
    @NonNull
    Dkim<?> deployDkim(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployDkim(@NonNull Command command) throws CommandRunException;
}
