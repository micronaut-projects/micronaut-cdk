...
import io.micronaut.cdk.action.spec.DynamicGroupSpec;
...
import io.micronaut.cdk.component.DynamicGroup;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link DynamicGroupSpec}.
 */
public interface DynamicGroupCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof DynamicGroupSpec)) {
            return null;
        }

        return deployDynamicGroup(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployDynamicGroup(command);
    }
     ...
     */
    @NonNull
    DynamicGroup<?> deployDynamicGroup(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployDynamicGroup(@NonNull Command command) throws CommandRunException;
}
