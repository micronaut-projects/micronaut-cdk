...
import io.micronaut.cdk.action.spec.EmailDomainSpec;
...
import io.micronaut.cdk.component.EmailDomain;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link EmailDomainSpec}.
 */
public interface EmailDomainCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof EmailDomainSpec)) {
            return null;
        }

        return deployEmailDomain(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployEmailDomain(command);
    }
     ...
     */
    @NonNull
    EmailDomain<?> deployEmailDomain(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployEmailDomain(@NonNull Command command) throws CommandRunException;
}
