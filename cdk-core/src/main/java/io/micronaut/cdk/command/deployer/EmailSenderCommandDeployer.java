...
import io.micronaut.cdk.action.spec.EmailSenderSpec;
...
import io.micronaut.cdk.component.EmailSender;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link EmailSenderSpec}.
 */
public interface EmailSenderCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof EmailSenderSpec)) {
            return null;
        }

        return deployEmailSender(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployEmailSender(command);
    }
     ...
     */
    @NonNull
    EmailSender<?> deployEmailSender(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployEmailSender(@NonNull Command command) throws CommandRunException;
}
