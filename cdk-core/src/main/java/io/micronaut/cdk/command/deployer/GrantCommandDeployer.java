...
import io.micronaut.cdk.action.delete.GrantDeleteSpec;
import io.micronaut.cdk.action.spec.GrantSpec;
...
import io.micronaut.cdk.component.Grant;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link GrantSpec}.
 */
public interface GrantCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof GrantSpec)) {
            return null;
        }

        return deployGrant(command);
    }

    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (!command.isDelete() || !(command.getDeleteSpec() instanceof GrantDeleteSpec)) {
            return false;
        }

        return undeployGrant(command);
    }
     ...
     */
    @NonNull
    Grant<?> deployGrant(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployGrant(@NonNull Command command) throws CommandRunException;
}
