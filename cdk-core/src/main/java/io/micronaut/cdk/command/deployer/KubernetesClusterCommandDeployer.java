...
import io.micronaut.cdk.action.spec.KubernetesClusterSpec;
...
import io.micronaut.cdk.component.KubernetesCluster;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link KubernetesClusterSpec}.
 */
public interface KubernetesClusterCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof KubernetesClusterSpec)) {
            return null;
        }

        return deployKubernetesCluster(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployKubernetesCluster(command);
    }
     ...
     */
    @NonNull
    KubernetesCluster<?> deployKubernetesCluster(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployKubernetesCluster(@NonNull Command command) throws CommandRunException;
}
