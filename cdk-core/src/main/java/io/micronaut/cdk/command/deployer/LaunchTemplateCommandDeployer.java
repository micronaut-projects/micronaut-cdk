...
import io.micronaut.cdk.action.spec.LaunchTemplateSpec;
...
import io.micronaut.cdk.component.LaunchTemplate;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link LaunchTemplateSpec}.
 */
public interface LaunchTemplateCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof LaunchTemplateSpec)) {
            return null;
        }

        return deployLaunchTemplate(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployLaunchTemplate(command);
    }
     ...
     */
    @NonNull
    LaunchTemplate<?> deployLaunchTemplate(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployLaunchTemplate(@NonNull Command command) throws CommandRunException;
}
