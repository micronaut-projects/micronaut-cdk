...
import io.micronaut.cdk.action.spec.LoadBalancerSpec;
...
import io.micronaut.cdk.component.LoadBalancer;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link LoadBalancerSpec}.
 */
public interface LoadBalancerCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof LoadBalancerSpec)) {
            return null;
        }

        return deployLoadBalancer(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployLoadBalancer(command);
    }
     ...
     */
    @NonNull
    LoadBalancer<?> deployLoadBalancer(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployLoadBalancer(@NonNull Command command) throws CommandRunException;
}
