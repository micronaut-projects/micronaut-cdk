...
import io.micronaut.cdk.action.delete.LogDeleteSpec;
import io.micronaut.cdk.action.spec.LogSpec;
...
import io.micronaut.cdk.component.Log;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link LogSpec}.
 */
public interface LogCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof LogSpec)) {
            return null;
        }

        return deployLog(command);
    }

    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (!command.isDelete() || !(command.getDeleteSpec() instanceof LogDeleteSpec)) {
            return false;
        }

        return undeployLog(command);
    }
     ...
     */
    @NonNull
    Log<?> deployLog(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployLog(@NonNull Command command) throws CommandRunException;
}
