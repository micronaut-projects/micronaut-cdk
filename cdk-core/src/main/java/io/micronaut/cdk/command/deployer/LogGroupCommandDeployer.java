...
import io.micronaut.cdk.action.spec.LogGroupSpec;
...
/**
 * Deployer for {@link LogGroupSpec}.
 */
public interface LogGroupCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof LogGroupSpec)) {
            return null;
        }

        return deployLogGroup(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployLogGroup(command);
    }
     ...
     */
    @NonNull
    LogGroup<?> deployLogGroup(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployLogGroup(@NonNull Command command) throws CommandRunException;
}
