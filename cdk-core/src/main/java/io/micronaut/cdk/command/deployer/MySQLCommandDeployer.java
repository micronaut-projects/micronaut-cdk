...
import io.micronaut.cdk.action.spec.MySQLSpec;
...
/**
 * Deployer for {@link MySQLSpec}.
 */
public interface MySQLCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof MySQLSpec)) {
            return null;
        }

        return deployMySQL(command);
    }
     ...
     */
    @NonNull
    MySQL<?> deployMySQL(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployMySQL(@NonNull Command command) throws CommandRunException;
}
