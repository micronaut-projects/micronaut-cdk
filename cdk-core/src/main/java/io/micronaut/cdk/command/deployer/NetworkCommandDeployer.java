...
import io.micronaut.cdk.action.spec.NetworkSpec;
...
/**
 * Deployer for {@link NetworkSpec}.
 */
public interface NetworkCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof NetworkSpec)) {
            return null;
        }

        return deployNetwork(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployNetwork(command);
    }
     ...
     */
    @NonNull
    Network<?> deployNetwork(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployNetwork(@NonNull Command command) throws CommandRunException;
}
