...
import io.micronaut.cdk.action.spec.PolicySpec;
...
/**
 * Deployer for {@link PolicySpec}.
 */
public interface PolicyCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof PolicySpec)) {
            return null;
        }

        return deployPolicy(command);
    }
     ...
     */
    @NonNull
    Policy<?> deployPolicy(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployPolicy(@NonNull Command command) throws CommandRunException;
}
