...
/**
 * Deployer for {@link ProjectSpec}.
 */
public interface ProjectCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof ProjectSpec)) {
            return null;
        }

        return deployProject(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployProject(command);
    }

    /**
     * Deploys a project to the cloud.
     *
     ...
     */
    @NonNull
    Project<?> deployProject(@NonNull Command command) throws CommandRunException;

    /**
     * Undeploys a project from the cloud.
     *
     * @param command the command
     * @return true if undeployed
     * @throws CommandRunException if there's a problem
     */
    boolean undeployProject(@NonNull Command command) throws CommandRunException;
}
