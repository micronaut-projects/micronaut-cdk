...
import io.micronaut.cdk.action.spec.RouteTableSpec;
...
import io.micronaut.cdk.component.RouteTable;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link RouteTableSpec}.
 */
public interface RouteTableCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof RouteTableSpec)) {
            return null;
        }

        return deployRouteTable(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployRouteTable(command);
    }
     ...
     */
    @NonNull
    RouteTable<?> deployRouteTable(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployRouteTable(@NonNull Command command) throws CommandRunException;
}
