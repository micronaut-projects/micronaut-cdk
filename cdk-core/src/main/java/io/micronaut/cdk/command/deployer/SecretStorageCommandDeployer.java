...
import io.micronaut.cdk.action.spec.SecretStorageSpec;
...
import io.micronaut.cdk.component.SecretStorage;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link SecretStorageSpec}.
 */
public interface SecretStorageCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof SecretStorageSpec)) {
            return null;
        }

        return deploySecretStorage(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeploySecretStorage(command);
    }
     ...
     */
    @NonNull
    SecretStorage<?> deploySecretStorage(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeploySecretStorage(@NonNull Command command) throws CommandRunException;
}
