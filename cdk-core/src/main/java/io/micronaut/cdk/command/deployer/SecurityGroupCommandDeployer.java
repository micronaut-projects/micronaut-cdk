...
import io.micronaut.cdk.action.spec.SecurityGroupSpec;
...
import io.micronaut.cdk.component.SecurityGroup;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link SecurityGroupSpec}.
 */
public interface SecurityGroupCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof SecurityGroupSpec)) {
            return null;
        }

        return deploySecurityGroup(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeploySecurityGroup(command);
    }
     ...
     */
    @NonNull
    SecurityGroup<?> deploySecurityGroup(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeploySecurityGroup(@NonNull Command command) throws CommandRunException;
}
