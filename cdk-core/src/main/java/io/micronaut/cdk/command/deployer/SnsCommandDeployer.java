...
import io.micronaut.cdk.action.spec.SnsSpec;
...
import io.micronaut.cdk.component.Sns;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link SnsSpec}.
 */
public interface SnsCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof SnsSpec)) {
            return null;
        }

        return deploySns(command);
    }
     ...
     */
    @NonNull
    Sns<?> deploySns(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeploySns(@NonNull Command command) throws CommandRunException;
}
