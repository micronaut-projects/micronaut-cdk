...
import io.micronaut.cdk.action.spec.StreamPoolSpec;
...
import io.micronaut.cdk.component.StreamPool;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link StreamPoolSpec}.
 */
public interface StreamPoolCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof StreamPoolSpec)) {
            return null;
        }

        return deployStreamPool(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployStreamPool(command);
    }
     ...
     */
    @NonNull
    StreamPool<?> deployStreamPool(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployStreamPool(@NonNull Command command) throws CommandRunException;
}
