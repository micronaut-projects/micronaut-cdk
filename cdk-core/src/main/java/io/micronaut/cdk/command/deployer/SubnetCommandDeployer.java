...
import io.micronaut.cdk.action.spec.SubnetSpec;
...
import io.micronaut.cdk.component.Subnet;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link SubnetSpec}.
 */
public interface SubnetCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof SubnetSpec)) {
            return null;
        }

        return deploySubnet(command);
    }
     ...
     */
    @NonNull
    Subnet<?> deploySubnet(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeploySubnet(@NonNull Command command) throws CommandRunException;
}
