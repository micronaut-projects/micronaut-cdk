...
import io.micronaut.cdk.action.spec.TargetGroupSpec;
...
import io.micronaut.cdk.component.TargetGroup;
import io.micronaut.cdk.util.Assert;
import io.micronaut.core.annotation.NonNull;

/**
 * Deployer for {@link TargetGroupSpec}.
 */
public interface TargetGroupCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof TargetGroupSpec)) {
            return null;
        }

        return deployTargetGroup(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployTargetGroup(command);
    }
     ...
     */
    @NonNull
    TargetGroup<?> deployTargetGroup(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployTargetGroup(@NonNull Command command) throws CommandRunException;
}
