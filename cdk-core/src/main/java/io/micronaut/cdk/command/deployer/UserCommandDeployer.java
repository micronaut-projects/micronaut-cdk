...
import io.micronaut.cdk.action.spec.UserSpec;
...
/**
 * Deployer for {@link UserSpec}.
 */
public interface UserCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof UserSpec)) {
            return null;
        }

        return deployUser(command);
    }
     ...
     */
    @NonNull
    User<?> deployUser(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployUser(@NonNull Command command) throws CommandRunException;
}
