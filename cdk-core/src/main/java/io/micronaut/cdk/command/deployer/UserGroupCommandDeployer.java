...
import io.micronaut.cdk.action.spec.UserGroupSpec;
...
/**
 * Deployer for {@link UserGroupSpec}.
 */
public interface UserGroupCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof UserGroupSpec)) {
            return null;
        }

        return deployUserGroup(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployUserGroup(command);
    }
     ...
     */
    @NonNull
    UserGroup<?> deployUserGroup(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployUserGroup(@NonNull Command command) throws CommandRunException;
}
