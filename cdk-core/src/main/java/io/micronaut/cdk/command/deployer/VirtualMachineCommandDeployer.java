...
import io.micronaut.cdk.action.spec.VirtualMachineSpec;
...
/**
 * Deployer for {@link VirtualMachineSpec}.
 */
public interface VirtualMachineCommandDeployer extends CommandDeployer {

    default Component<?> deploy(@NonNull Command command) throws CommandRunException {
        Assert.notNull(command, "Command cannot be null");

        if (command.isUpdate() || !(command.getSpec() instanceof VirtualMachineSpec)) {
            return null;
        }

        return deployVirtualMachine(command);
    }
    ...
    default boolean undeploy(@NonNull Command command) throws CommandRunException {
        ...
        return undeployVirtualMachine(command);
    }
     ...
     */
    @NonNull
    VirtualMachine<?> deployVirtualMachine(@NonNull Command command) throws CommandRunException;
     ...
     * @throws CommandRunException if there's a problem
     */
    boolean undeployVirtualMachine(@NonNull Command command) throws CommandRunException;
}
